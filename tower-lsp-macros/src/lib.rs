...
pub fn server_rpc(_attr: TokenStream, item: TokenStream) -> TokenStream {
...
pub fn rpc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    match attr_args.as_slice() {
        [NestedMeta::Meta(meta)] if meta.path().is_ident("name") => item,
...
/// as RPC handlers.
#[proc_macro_attribute]
pub fn client_rpc(_attr: TokenStream, item: TokenStream) -> TokenStream {
    let lang_server_trait = parse_macro_input!(item as ItemTrait);
    let method_calls = parse_method_calls(&lang_server_trait);
    let req_types_and_router_fn = gen_client_router(&lang_server_trait.ident, &method_calls);
...
fn gen_server_router(trait_name: &syn::Ident, methods: &[MethodCall]) -> proc_macro2::TokenStream {
    ...
    quote! {
        mod generated_server {
            ...
            use crate::service::{layers, Client, ServerToClient, Pending, ServerState, State, ExitedError};
...
fn gen_client_router(trait_name: &syn::Ident, methods: &[MethodCall]) -> proc_macro2::TokenStream {
    let route_registrations: proc_macro2::TokenStream = methods
        .iter()
        .map(|method| {
            let rpc_name = &method.rpc_name;
            ...
            let layer = quote! { layers::Normal::new(state.clone(), pending.clone()) };
    ...
    quote! {
        mod generated_client {
            ...
            use super::#trait_name;
            use crate::jsonrpc::{Result, Router};
            use crate::service::{layers, Client, ClientToServer, Pending, ServerState, State, ExitedError};
